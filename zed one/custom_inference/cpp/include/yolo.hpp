...
public:
    ...
    std::vector<BBoxInfo> run(sl::Tensor& input_tensor, int orig_image_h, int orig_image_w, float thres);

    sl::Resolution getInferenceSize() {
        return sl::Resolution(input_width, input_height);
    }

    // Get tensor parameters configured for this YOLO model
    sl::TensorParameters getTensorParameters() const {
        sl::TensorParameters params;
        params.target_size = sl::Resolution(input_width, input_height);
        params.batch_size = 1;
        params.pixel_type = sl::TensorParameters::PIXEL_TYPE::FLOAT;
        params.layout = sl::TensorParameters::LAYOUT::NCHW;
        params.color_format = sl::TensorParameters::COLOR_FORMAT::RGB;
        params.memory_type = sl::MEM::GPU;
        params.scale = sl::float3(1.0f / 255.0f, 1.0f / 255.0f, 1.0f / 255.0f);
        params.mean = sl::float3(0, 0, 0);
        params.std = sl::float3(1, 1, 1);
        params.stretch = false; // Keep aspect ratio (letterboxing)
        return params;
    }
...
private:
    ...
    size_t input_width = 640, input_height = 640, batch_size = 1;
