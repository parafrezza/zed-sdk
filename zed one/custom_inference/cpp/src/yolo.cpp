...
std::vector<BBoxInfo> Yolo::run(sl::Tensor& input_tensor, int orig_image_h, int orig_image_w, float thres) {
    std::vector<BBoxInfo> binfo;

    // Input tensor is already preprocessed by retrieveTensor - just use it directly
    float* input_ptr = input_tensor.getPtr<float>(sl::MEM::GPU);
    if (!input_ptr) {
        std::cerr << "Invalid input tensor pointer" << std::endl;
        return binfo;
    }

#if (NV_TENSORRT_MAJOR < 8) || (NV_TENSORRT_MAJOR == 8 && NV_TENSORRT_MINOR < 5)
    std::vector<void*> d_buffers_nvinfer(2);
    d_buffers_nvinfer[inputIndex] = input_ptr;
    d_buffers_nvinfer[outputIndex] = d_output;
    context->enqueueV2(&d_buffers_nvinfer[0], stream, nullptr);
#else
    context->setTensorAddress(input_binding_name.c_str(), input_ptr);
