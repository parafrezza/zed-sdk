...
// Simple 3D object for line/box rendering (no CUDA, pure GL)
...
public:
    ...
    void init();
    void clear();
    void addLine(const sl::float3& p1, const sl::float3& p2, const sl::float4& color);
    void addBox(const std::vector<sl::float3>& corners, const sl::float4& color);
    void addSphere(const sl::float3& center, float radius, const sl::float4& color, int segments = 8);
    void addCylinder(const sl::float3& p1, const sl::float3& p2, float radius, const sl::float4& color, int segments = 6);
    void pushToGPU();
...
// Pending detected objects for rendering
struct PendingObjectsData {
    sl::Objects objects;
    bool hasNew = false;
};

// Pending body tracking data for rendering
struct PendingBodiesData {
    sl::Bodies bodies;
    bool hasNew = false;
};
...
private:
    ...
    // Pending detection data
    std::unordered_map<std::string, PendingObjectsData> pendingObjects_;
    std::unordered_map<std::string, PendingBodiesData> pendingBodies_;

    // 3D rendering objects for detections
    Simple3DObject objectBoxes_;
    Simple3DObject bodySkeletons_;
