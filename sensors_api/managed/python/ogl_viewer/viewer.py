"""
OpenGL viewer for point cloud visualization.
Supports multiple point clouds from ZED cameras and LiDAR sensors.
"""
...
# Color palette for object IDs (RGBA, values 0-1)
ID_COLORS = np.array([
    [0.231, 0.596, 0.882, 1.0],
    [0.965, 0.263, 0.412, 1.0],
    [0.098, 0.686, 0.816, 1.0],
    [0.412, 0.4, 0.804, 1.0],
    [1.0, 0.725, 0.0, 1.0],
    [0.989, 0.388, 0.419, 1.0]
], np.float32)


def generate_color_id(_idx):
    """Generate a color for a given object ID."""
    if _idx < 0:
        return [236 / 255.0, 184 / 255.0, 36 / 255.0, 1.0]
    offset = _idx % len(ID_COLORS)
    return [ID_COLORS[offset][0], ID_COLORS[offset][1], ID_COLORS[offset][2], 1.0]
...
# Try to enable GPU acceleration with CuPy and CUDA-OpenGL interop
try:
...
class Simple3DObject:
    ...
    def add_full_edges(self, _pts, _clr):
        """Add top and bottom wireframe edges for a 3D bounding box (8 corners)."""
        ...
        for i in range(len(_pts)):
            self.add_pt(_pts[i])
            self.add_clr(_clr)
        box_links_top = [0, 1, 1, 2, 2, 3, 3, 0]
        for i in range(0, len(box_links_top), 2):
            self.indices.append(start_id + box_links_top[i])
            self.indices.append(start_id + box_links_top[i + 1])
        box_links_bottom = [4, 5, 5, 6, 6, 7, 7, 4]
        for i in range(0, len(box_links_bottom), 2):
    ...
    def add_vertical_faces(self, _pts, _clr):
        """Add semi-transparent side faces with vertical gradient."""
        quads = [[0, 3, 7, 4], [3, 2, 6, 7], [2, 1, 5, 6], [1, 0, 4, 5]]
        alpha = 0.25
        for quad in quads:
            quad_pts_1 = [
                _pts[quad[0]], _pts[quad[1]],
    ...
    def init(self, width, height):
        ...
        self.elementbufferSize = width * height
...
class GLViewer:
    """OpenGL viewer for multi-sensor point cloud visualization."""
    ...
    def __init__(self):
        ...
        self.point_clouds = {}  # Dictionary of sensor_name -> Simple3DObject
        self.BBox_edges = Simple3DObject(False, 3, 4)
        self.BBox_faces = Simple3DObject(False, 3, 4)
        self.pending_objects = {}  # sensor_name -> sl.Objects
        self.window_name = b"Sensors API - Point Cloud"

    def init(self, window_name: str = "Sensors API - Point Cloud"):
        self.window_name = window_name.encode() if isinstance(window_name, str) else window_name

        glutInit(sys.argv)
        wnd_w = int(glutGet(GLUT_SCREEN_WIDTH) * 0.7)
        wnd_h = int(glutGet(GLUT_SCREEN_HEIGHT) * 0.7)
        glutInitWindowSize(wnd_w, wnd_h)
        glutInitWindowPosition(int(wnd_w * 0.1), int(wnd_h * 0.1))
        ...
        self.bckgrnd_clr = np.array([0.1, 0.1, 0.1])

        # Create camera frustum model
        Z_ = -0.15
    ...
    def updateData(self, pc, sensor_name: str = "default"):
        """Update point cloud from sl.Mat for a given sensor."""
        self.mutex.acquire()
        try:
            if sensor_name not in self.point_clouds:
                # Size buffer from actual point cloud dimensions, not hardcoded values
                w = pc.get_width()
                h = pc.get_height()
                point_cloud = Simple3DObject(False, 4)
                point_cloud.init(w, h)
                point_cloud.set_drawing_type(GL_POINTS)
                self.point_clouds[sensor_name] = point_cloud

            self.point_clouds[sensor_name].setPoints(pc)
        finally:
            self.mutex.release()

    def updateObjects(self, objects, sensor_name: str = "default"):
        """Update detected objects for 3D bounding box rendering."""
        self.mutex.acquire()
        try:
            self.pending_objects[sensor_name] = objects
        finally:
            self.mutex.release()
    ...
    def draw(self):
        ...
        # Rebuild bounding box geometry from pending objects
        self.BBox_edges.clear()
        self.BBox_faces.clear()
        for sensor_name, objects in self.pending_objects.items():
            for obj in objects.object_list:
                if obj.tracking_state == sl.OBJECT_TRACKING_STATE.OK:
                    bounding_box = np.array(obj.bounding_box)
                    if bounding_box.shape[0] >= 8:
                        color_id = generate_color_id(obj.id)
                        self.BBox_edges.add_full_edges(bounding_box, color_id)
                        self.BBox_edges.add_vertical_edges(bounding_box, color_id)
                        self.BBox_faces.add_vertical_faces(bounding_box, color_id)
                        self.BBox_faces.add_top_face(bounding_box, color_id)
